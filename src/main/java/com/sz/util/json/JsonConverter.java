...
public class JsonConverter {
	...
	public static JSONObject object2Json(Object bean) throws JsonConvertException {
		JSONObject json=new JSONObject(generalSendObject(bean));
		return json;
	}
	...
	public static <T> T json2Object(
			JSONObject jsonObj, Class<T> messageClass)
			throws JsonConvertException {
		...
		try {
			...
			for (Iterator iterator = jsonObj.keys(); iterator.hasNext();) {
				...
				if (null == fieldStructsBean) {
					log.error("Can't find field named {} in class {}" + alias
							+ messageClass.getName());
					continue;
				}
				// System.out.println("field-alias="+alias);
				...
				if (null != fieldValue
					...
					fieldValue = baseTypeConverter(fieldValue, fieldType);
				}
	...
	private static Object baseTypeConverter(Object fieldValue, Class fieldType) {
	...
	public static void setChar(char a) {
		System.out.println(a);
	}
	...
	private static Map generalSendObject(Object bean) throws JsonConvertException {
	 ...
	 * @throws JsonConvertException
	 */
	private static Map<String, Object> convertObj2Map(Object bean, Map<String, FieldStructsBean> attributeMapping) throws JsonConvertException {
		// instance a map
		...
		try {
			for (String fieldName : attributeKeySet) {
				...
				Method getMethod;
				getMethod = beanClass.getMethod(getMethodName);
				// normal field value
	...
	private static Collection generalSendCollection(Collection oriColl) throws JsonConvertException{
		//list
		Collection coll = new ArrayList();
		//vector
		if(Vector.class.isAssignableFrom(oriColl.getClass())){
			coll = new Vector();
		}
		//set
		else if(Set.class.isAssignableFrom(oriColl.getClass())){
			coll = new HashSet();
		}
	...
	private static Object[] generalSendArray(Object[] oriObjArr)
			throws JsonConvertException {
		if (null != oriObjArr && oriObjArr.length > 0) {
			Object[] obj = new Object[oriObjArr.length];
			for (int i = 0; i < oriObjArr.length; i++) {
				if(null != oriObjArr[i]){
					obj[i] = oriObjArr[i];
					if(Collection.class.isAssignableFrom(oriObjArr[i].getClass())){
						obj[i] = generalSendCollection((Collection)oriObjArr[i]);
					}else if(Map.class.isAssignableFrom(oriObjArr[i].getClass())){
						obj[i] = generalSendMap((Map)oriObjArr[i]);
					}else if(oriObjArr[i].getClass().isArray()){
						obj[i] = generalSendArray((Object[])oriObjArr[i]);
